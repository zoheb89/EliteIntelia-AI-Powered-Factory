...
TYPE_SIGNALS: Dict[str, List[Tuple[str, float]]] = {
    "rfp": [(r"\brequest for proposal\b", 6), (r"\brfp\b", 4), (r"\bmandatory requirement", 3), (r"\bevaluation criteria\b", 3), (r"\bsubmission (?:requirement|instruction|deadline)", 3), (r"\bbidder|tenderer\b", 2), (r"\bscope of work\b", 2), (r"\bservice level agreement|\bsla\b", 1.5)],
    "rfi": [(r"\brequest for information\b", 6), (r"\brfi\b", 4), (r"\binformation (?:requested|required)\b", 3), (r"\bcapability (?:statement|questionnaire)\b", 3), (r"\bplease describe\b", 2), (r"\bvendor questionnaire\b", 2)],
    "rfq": [(r"\brequest for quot", 6), (r"\brfq\b", 4), (r"\bunit price|\bprice schedule\b", 3), (r"\bquantit(?:y|ies)\b", 2), (r"\bquotation\b", 3), (r"\bdelivery terms\b", 2)],
    "sow": [(r"\bstatement of work\b", 6), (r"\bsow\b", 4), (r"\bdeliverable", 2), (r"\bacceptance criteria\b", 3), (r"\bmilestone", 2), (r"\bchange control\b", 2)],
    "architecture": [(r"\barchitecture (?:diagram|document|overview)\b", 5), (r"\bdata flow\b", 2), (r"\bcomponent diagram\b", 3), (r"\bhigh[- ]level design\b", 4), (r"\btarget state\b", 2)],
    "schema": [(r"\bcreate table\b", 5), (r"\bprimary key\b", 3), (r"\bforeign key\b", 3), (r"\bdata dictionary\b", 5), (r"\bcolumn name\b", 2), (r"\bvarchar|nvarchar|int\b", 1)],
    "meeting_notes": [(r"\bmeeting (?:notes|minutes)\b", 6), (r"\battendees\b", 4), (r"\baction items?\b", 3), (r"\bagenda\b", 2), (r"\bnext steps\b", 2), (r"\bworkshop\b", 3)],
    "requirements": [(r"\bfunctional requirement", 5), (r"\bnon[- ]functional requirement", 5), (r"\buser stor(?:y|ies)\b", 3), (r"\bshall\b", 1.5), (r"\bacceptance criteria\b", 2)],
}

SENSITIVITY_SIGNALS: Dict[str, List[str]] = {
    "phi": [r"\bpatient\b", r"\bdiagnos", r"\bmedical record\b", r"\bclinical\b", r"\bprescription\b", r"\bhipaa\b", r"\bprotected health information\b", r"\badmission\b", r"\bdischarge\b"],
    "pii": [r"\bdate of birth\b", r"\bnational id\b", r"\bpassport\b", r"\bssn\b", r"\bemail address\b", r"\bphone number\b", r"\bgdpr\b", r"\bpersonally identifiable\b", r"\baddress\b"],
}
...
class Classification:
    ...
    def to_dict(self) -> dict:
        return {"document_type": self.document_type, "confidence": self.confidence, "scores": {k: round(v, 1) for k, v in self.scores.items() if v}, "sensitivity": self.sensitivity, "sensitivity_signals": self.sensitivity_signals, "signals_matched": self.signals_matched[:12]}

def sha256(data: bytes) -> str:
    return hashlib.sha256(data).hexdigest()

def classify(text: str, filename: str = "") -> Classification:
    body = (text or "").lower(); name = (filename or "").lower(); scores: Dict[str, float] = {}; matched: List[str] = []
    for doc_type, signals in TYPE_SIGNALS.items():
        total = 0.0
        for pattern, weight in signals:
            hits = len(re.findall(pattern, body))
            if hits:
                total += weight * min(hits, 4); matched.append(f"{doc_type}:{pattern.strip(chr(92) + 'b')}")
    ...
    for level, patterns in SENSITIVITY_SIGNALS.items():
        ...
        if len(hits) >= 2:
            sensitivity = level; sens_signals = [h.strip(chr(92) + "b") for h in hits[:6]]; break
...
def extract_explicit_intent(text: str) -> str:
    ...
    if not body: return ""
    patterns = [
        r"(?:the\s+)?(?:purpose|objective|goal)\s+(?:of\s+(?:this\s+)?(?:rfp|project|initiative)\s+)?(?:is|are|:)\s+(.{40,500}?)(?=\.\s+|$)",
        r"(?:this\s+(?:rfp|project|initiative))\s+(?:seeks|aims|is\s+intended)\s+to\s+(.{40,500}?)(?=\.\s+|$)",
        r"(?:we\s+(?:seek|are\s+looking)\s+to|the\s+requirement\s+is\s+to)\s+(.{40,500}?)(?=\.\s+|$)",
    ]
    for pattern in patterns:
        ...
        if match:
            ...
            if len(value) >= 40:
                return value[:500].rstrip(" ,;:") + ("." if not value.endswith((".", "!", "?")) else "")
    return ""
...
def ingest(repo, project_id: str, filename: str, data: bytes, mime_type: str = "", source: str = "upload") -> Dict[str, Any]:
    ...
    if existing:
        return {"duplicate": True, "evidence_id": existing.id, "name": existing.name, "document_type": existing.document_type, "message": "This file has already been ingested; the existing record was kept."}
    ...
    if intent and project and not (project.intent or "").strip():
        ...
        repo.audit("intent.captured", "stage", "intent", reason="Explicit customer intent captured from uploaded evidence.", after={"evidence_id": ev.id, "text": intent[:300]}, actor_kind="system", project_id=project_id)
